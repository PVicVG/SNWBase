/// player_handle_monitors(Side)
 ...
 // Check if we collide with the top:
    if(argument0 == "top")
    {
       ...
       if((monitor && y_speed >= 0) &&
       (state == STATE_JUMP || state == STATE_FANG_BOUNCE || 
       state == STATE_ROLL || state == STATE_DEFAULT && animation = "roll" || 
       shield_state == 1 || state == STATE_KICK || state == STATE_HOMING || state == STATE_STOMP || state == STATE_STOMP_LAND)
       && state != STATE_HURT)
       {
          // Destroy monitor:
 ...
 // Check if we collide with the side: 
    if(argument0 == "side")
    {
       ...
       if(monitor != noone && (state == STATE_ROLL || state == STATE_KICK || shield_state == 1 || state == STATE_GLIDE || state == STATE_SLIDE || state == STATE_HOMING) && (player_collision_check(COL_LEFT_OBJECT, MASK_BIG, x, y, angle, monitor) || player_collision_check(COL_RIGHT_OBJECT, MASK_BIG, x, y, angle, monitor)))
       {
          // Destroy monitor:
             monitor.parent = obj_player;
             with(monitor)
 ...
 // Check if we collide with the bottom: 
    var monitor;
    monitor = instance_nearest(x, y, obj_monitor);
    if(monitor != noone && (y_speed <= 0 && (state == STATE_DEFAULT || state == STATE_JUMP || state == STATE_ROLL || state == STATE_HOMING || animation = "roll")) && (player_collision_check(COL_TOP_OBJECT, MASK_BIG, x, y, angle, monitor)))
    {  
